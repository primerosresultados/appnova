...
import { Plus, TrendingUp, TrendingDown, DollarSign, Wallet, FileText, ArrowUpRight, ArrowDownRight } from "lucide-react";
...
async function getFinanceData() {
    ...
    // Calculate Monthly Cash Flow (Mock calculation logic for now)
    const incomeThisMonth = await db.transaction.aggregate({
        where: { type: 'INCOME' },
        _sum: { amount: true }
    });

    const expensesThisMonth = await db.transaction.aggregate({
        where: { type: 'EXPENSE' },
        _sum: { amount: true }
    });

    const taxPayload = await db.transaction.aggregate({
        where: { isTaxable: true, type: 'INCOME' }, // Simplified IVA Logic
        _sum: { taxAmount: true }
    });

    const allFinancialRecords = await db.financialRecord.findMany({
        orderBy: { date: 'desc' },
        include: { client: true }
    });
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                ...
                <div className="flex gap-3">
                    <Button variant="outline" className="h-10 border-border/50 bg-background/50 backdrop-blur-sm">
                        <FileText className="h-4 w-4 mr-2" /> Reporte SII
                    </Button>
                    ...
                    <div className="grid gap-6 md:grid-cols-12">
                        ...
                        <Card className="md:col-span-4 bg-card/30 backdrop-blur-md border border-border/40 shadow-xl">
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <Button variant="outline" className="w-full text-xs" size="sm">Gestionar Cuentas</Button>
                            </CardContent>
                        </Card>
